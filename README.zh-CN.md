...
- 文件夹需要先被信任才会使用：未信任的文件夹既不会被读写，它声明的 MCP 服务（`mcp_config.json` / `.mcp.json`）也不会被启动。所有 `fs:*` IPC 处理都会把路径与已信任的根重新比对，并解析符号链接，使工作区内的链接无法指向工作区之外；信任只能由主进程自己弹出的原生确认框授予。
- 非安装包自带的 MCP 服务，每次工具调用都需要你逐次批准。
- 渲染进程使用严格的 Content-Security-Policy。
- API Key 以 AES-256-GCM 加密存储，密钥由本机 machine-id 与一个固定盐派生。它能防住的是数据库文件被拷走，仅此而已——以你的用户身份运行的程序可以做同样的派生。
- 聊天记录、工具输出与模型请求日志在应用数据目录中以明文存放。
- 聊天中的 Markdown 内容经 DOMPurify 消毒后再渲染。
